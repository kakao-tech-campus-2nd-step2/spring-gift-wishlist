...
public class JwtProvider {
    ...
    public static String generateToken(User user) {
        return Jwts.builder().claim("email", user.getEmail()).claim("id", user.getId())
    ...
    public UserInfo getClaim(String token) {
        Claims claimsBody = Jwts.parser().setSigningKey(SECRET_KEY).build()
            .parseClaimsJws(removeBearer(token)).getBody();

        UserInfo currentUser = new UserInfo(claimsBody.get("email").toString(),
