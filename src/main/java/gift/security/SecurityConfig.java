        ...
        http
            ...
            .exceptionHandling(exceptionHandling -> exceptionHandling // 예외 처리 exception handling
                .authenticationEntryPoint((request, response, authException) -> { // 인증 과정에서 실패 시
                    response.setStatus(
                        HttpServletResponse.SC_UNAUTHORIZED); // HTTP 상태 코드를 401로 설정한다.
            ...
            .authorizeHttpRequests(authz -> authz // 요청 규직 인증
                    .requestMatchers("/members/register", "/members/login", "/", "/h2-console/**",
                        "/h2-console/login.do?**").permitAll() // register/login은 요청은 인증 없이 허용된다.
                    .anyRequest().authenticated()
                // 위에서 명시적으로 허용된 경로를 제외한 모든 다른 요청은 인증된 사용자만 접근, 사용자는 다른 request를 위해 로그인 해야됨
            )
